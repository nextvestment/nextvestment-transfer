...
async function editText(page: Page, value: string) {
  ...
  // Exercise keyboard input after focus, rather than mutating a Monaco model
  // before React has finished attaching its editor lifecycle callbacks.
  await page.locator('.monaco-editor .view-lines').click();
  // Device presets can emulate a different OS from the test runner. Monaco
  // selects its keymap from the browser user agent, not process.platform.
...
test('bucket favorites reopen the root and can be removed from the sidebar', async ({ page }) => {
  ...
  // Account-wide bucket discovery is opt-in; populate the sidebar explicitly.
