...
test('disabled discovery clears an explicitly fetched list on profile change without listing the new account', async () => {
  const view = renderHook(() => useBuckets({ enabled: false }));
  await act(async () => view.result.current.fetchBuckets());
  expect(view.result.current.buckets.length).toBeGreaterThan(0);
  const calls = vi.mocked(bucketApi.listBucketsWithRegions).mock.calls.length;
  act(() => useProfileStore.getState().setActiveProfileId('another'));
  await waitFor(() => expect(view.result.current.buckets).toEqual([]));
  expect(bucketApi.listBucketsWithRegions).toHaveBeenCalledTimes(calls);
});
