...
pub(crate) async fn load_sdk_config(
    ...
    match &profile.credential_type {
        CredentialType::IdentityCenter { .. } => {
            // Build explicitly: no ambient shared-config or endpoint overrides.
            aws_config::SdkConfig::builder()
                .behavior_version(aws_config::BehaviorVersion::latest())
                .region(region)
                .credentials_provider(
                    aws_credential_types::provider::SharedCredentialsProvider::new(
                        crate::credentials::identity_center::IdentityCenterProvider::new(
                            profile.clone(),
                        ),
                    ),
                )
...
impl S3ClientManager {
    ...
    pub async fn get_client_for_region(
        ...
        // A signed-out native session cannot continue through a cached S3 client.
        crate::credentials::identity_center::validate_profile_binding(profile)?;
